...
export class Color_Selector {
	...
	static color_vec_to_str(color) {
		return `#${color[0].toString(16).padStart(2, '0')}${color[1].toString(16).padStart(2, '0')}${color[2]
			.toString(16)
			.padStart(2, '0')}`;
	}
	...
	render_and_attach_html(parent_element, data = {}) {
		...
		this.hue_sidebar_context = this.hue_sidebar.getContext('2d', {
			willReadFrequently: true,
		});
	...
	initialize_hue_sidebar() {
		const vertical_gradient = this.hue_sidebar_context.createLinearGradient(
			0,
			0,
			0,
			this.hue_sidebar_context.canvas.height
		);
		...
		this.hue_sidebar_context.fillRect(
			0,
			0,
			this.hue_sidebar_context.canvas.width,
			this.hue_sidebar_context.canvas.height
		);
	}
	...
	initialize_dropper() {
		...
		const ls = [];
		for (let i = 0; i < this.options.dropper_read_size; i++)
			ls.push(pixel_id_list.slice(i * this.options.dropper_read_size, (i + 1) * this.options.dropper_read_size));
	...
	rgb_input_handler(e) {
		const red = Color_Selector.partial_hex_test.test(this.red_input.value)
			? this.red_input.value.padStart(2, '0')
			: '00';
		const green = Color_Selector.partial_hex_test.test(this.green_input.value)
			? this.green_input.value.padStart(2, '0')
			: '00';
	...
	hue_click_handler(e) {
		...
		this.change_canvas_gradients(
			Color_Selector.color_vec_to_str(Color_Selector.hsv_vec_to_rgb([this.get_hue(), 1, 1]))
		);
		this.update_html_colors(this.get_canvas_color(), 'canvas');
	}
	...
	update_canvas(color) {
		...
		this.move_canvas_marker(
			...
			Math.round(this.options.canvas_size * (1 - hsv[2]))
		);
