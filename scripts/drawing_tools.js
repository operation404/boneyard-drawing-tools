...
export class Drawing_Tools_2 extends Application {

    static init() {
        Drawing_Tools_2.prepare_hook_handlers();
        console.log(`====== Boneyard ======\n - Drawing tools initialized`);
    }

    static prepare_hook_handlers() {
        if (game.settings.get(MODULE, SETTING_SIDEBAR_BUTTONS)) {
            Hooks.on("getSceneControlButtons", (controls) => Drawing_Tools_2.add_control_buttons(controls));
        }
    }
    ...
    static add_control_buttons(controls) {
        ...
        drawing_controls.tools.push({
            ...
            "onClick": () => {
                new Drawing_Tools_2().render(true); // TODO should this be defaulted to stroke?
            },
            button: true,
        });
    }
    ...
    constructor(options = {}) {
        super(options);
        this.current_tool = game.settings.get(MODULE, LAST_TOOL);
    }
    ...
    getData() {
        ...
        return { // Send data to the html template
            ...
            current_tool_tooltip: game.i18n.localize(`CONTROLS.${this.current_tool}_color`),
    ...
    async _render(force = false, options = {}) {
        ...
        const color_text_input = this._element[0].querySelector("#by-quick-draw-config #by-stroke-color-text");
        color_text_input.setSelectionRange(color_text_input.value.length, color_text_input.value.length);
        color_text_input.focus();
    }
    ...
    activateListeners(html) {
        ...
        document.querySelector("#by-quick-draw-config #by-stroke-alpha")
            .addEventListener("change", (e) => {this.alpha_slider_handler(e);});
        ...
        // TODO still need handlers for the -/+ buttons
    ...
    color_button_handler(e) {
        ...
        game.settings.set(MODULE, LAST_TOOL, tool);
        //TODO this shouldn't be a setting that's remembered across sessions
        // the current tool should only persist for the current session
        // set it to stroke during init and change it here as needed, but don't save it to a setting
    }

    static hex_test = /^#[0-9A-F]{6}$/i;
    color_text_handler(e) {
        const tool = e.target.dataset.tool;
        const update_color = Drawing_Tools_2.hex_test.test(e.target.value) ? e.target.value : '#000000';
        document.querySelector(`#by-quick-draw-config #by-${tool}-color`).style.setProperty("background", update_color);
